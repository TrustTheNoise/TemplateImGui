...
void ImGui::BulletTextV(const char* fmt, va_list args)
{
    ...
    const ImVec2 total_size = ImVec2(
        g.FontSize
            + (label_size.x > 0.0f ? (label_size.x + style.FramePadding.x * 2)
                                   : 0.0f),
    ...
    RenderBullet(window->DrawList,
                 bb.Min
                     + ImVec2(style.FramePadding.x + g.FontSize * 0.5f,
                              g.FontSize * 0.5f),
                 text_col);
                           ...
                           bool* out_held, ImGuiButtonFlags flags)
{
    ...
    ImGuiItemFlags item_flags
        = (g.LastItemData.ID == id ? g.LastItemData.InFlags
                                   : g.CurrentItemFlags);
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags, float thickness)
{
    ...
    if (flags & ImGuiSeparatorFlags_Vertical) {
        ...
        // FIXME-WORKRECT: In theory we should simply be using
        // WorkRect.Min.x/Max.x everywhere but it isn't aesthetically what we
        // want, need to introduce a variant of WorkRect for that purpose.
        // (#4787)
                         ...
                         const char* format, ImGuiSliderFlags flags)
{
    ...
    IM_ASSERT((flags == 1 || (flags & ImGuiSliderFlags_InvalidMask_) == 0)
              && "Invalid ImGuiSliderFlags flags! Has the 'float power' "
                        ...
                        = (mouse_abs_pos >= grab_pos - grab_sz * 0.5f - 1.0f)
                          && (mouse_abs_pos
                              <= grab_pos + grab_sz * 0.5f
                                     + 1.0f); // No harm being extra generous
                                              // here.
                           ...
                           ImRect* out_grab_bb)
{
    ...
    // assert.
    IM_ASSERT((flags == 1 || (flags & ImGuiSliderFlags_InvalidMask_) == 0)
              && "Invalid ImGuiSliderFlags flag!  Has the 'float power' "
                 "argument been mistakenly cast to flags? Call function with "
                 "ImGuiSliderFlags_Logarithmic flags instead.");
                                     ...
                                     ImGuiInputSource input_source)
{
    ...
    if (c < 0x20) {
        ...
        pass |= (c == '\n'
                 && (flags
                     & ImGuiInputTextFlags_Multiline)); // Note that an Enter
                                                        // KEY will emit \r and
                                                        // be ignored (we poll
                                                        // for KEY in
                                                        // InputText() code)
                        ...
                        void* callback_user_data)
{
    ...
    bool render_selection
        = state && (state->HasSelection() || select_all)
                       ...
                       ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    const bool item_add = ItemAdd(bb, id, NULL,
                                  disabled_item ? ImGuiItemFlags_Disabled
                                                : ImGuiItemFlags_None);
    if (span_all_columns) {
...
bool ImGui::BeginMenuBar()
{
    ...
    ImRect clip_rect(
        ...
        IM_ROUND(ImMax(bar_rect.Min.x, bar_rect.Max.x
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
        ...
        RenderArrow(
            window->DrawList,
            pos
                + ImVec2(offsets->OffsetMark + extra_w + g.FontSize * 0.30f,
                         0.0f),
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    for (int section_n = 0; section_n < 3; section_n++) {
        ...
        for (int tab_n = 0; tab_n < section->TabCount; tab_n++) {
            ...
            tab_offset
                += tab->Width
                   + (tab_n < section->TabCount - 1 ? g.Style.ItemInnerSpacing.x
                                                    : 0.0f);
        }
                         ...
                         ImGuiTabItemFlags flags)
{
    ...
    IM_ASSERT(
        !(flags & ImGuiTabItemFlags_Button)); // BeginTabItem() Can't be used
                                              // with button flags, use
                                       ...
                                       bool* out_text_clipped)
{
    ...
    const ImVec2 button_pos(
        ImMax(bb.Min.x, bb.Max.x - frame_padding.x * 2.0f - button_sz),
        bb.Min.y);
